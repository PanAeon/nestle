...
const Output = packed struct(u8) { S: bool = false, E: bool = false, M: bool = false, _: u5 = 0 };
    ...
    switch (address) {
        0x4016 => {
            if (self.numReads1 < 8) {
                ...
                const shift: u3 = @truncate(self.numReads1);
        ...
        0x4017 => {
            if (self.numReads2 < 8) {
                ...
                const shift: u3 = @truncate(self.numReads2);
                const mask: u8 = @as(u8, 1) << shift;
                out.S = ((bits & mask) >> shift) == 1;
                out._ = 0b10000;
                self.numReads2 += 1;
